"""
Shared Firestore/GCS clients for the training pipeline, authenticated as
training-job-runtime via short-lived impersonation - the same pattern
server/src/gcs.ts uses for api-runtime. One code path works whether the
ambient credentials already ARE training-job-runtime (Cloud Run Job, prod -
effectively self-impersonation) or a user account with
roles/iam.serviceAccountTokenCreator on it (local dev, this machine).

Required env vars (see pipeline/train/.env.example):
